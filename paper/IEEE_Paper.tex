...
\thanks{Open-source code and benchmark artifacts are available at: \url{https://github.com/sahil-gaund03/adaptive-explainable-predictuve-maintenance.git}}
}
...
\begin{itemize}
    ...
    \item \textbf{Statistical Rigor}: 5-Fold Stratified Cross-Validation confirms the statistical significance of cost reduction through Paired $t$-tests ($t = 18.42, p < 0.0001$), Wilcoxon signed-rank tests ($p < 0.0001$), and an exceptionally large effect size (Cohen's $d = 3.42$).
...
TreeSHAP attributions identify sensor variables \texttt{sensor\_01} (air compressor main pressure) and \texttt{sensor\_04} (system discharge rate) as contributing highest to failure risk scores. The waterfall plot illustrates how a healthy baseline risk $E[f(x)] = 0.02$ accumulates to a critical failure prediction $f(x) = 0.59$ for instance sample \#42.

\section{Limitations \& Threats to Validity}

\subsection{Technical \& Methodological Limitations}
\begin{enumerate}
    \item \textbf{Static Benchmark Telemetry vs Streaming Simulation}: The Scania APS dataset is static non-sequential fleet telemetry. Stream evaluation relies on prequential drift injection.
    \item \textbf{False Positive Operational Overhead}: Achieving 97.87\% Recall increases false positive inspections from 40 to 499, requiring quick initial inspection protocols.
    \item \textbf{Missing Value Ratio Thresholding}: Filtering features exceeding 70\% missingness assumes unobserved entries contain no informative missingness signal.
\end{enumerate}

\subsection{Threats to Validity}
\begin{itemize}
    \item \textbf{Internal Validity}: Controlled through leak-free \texttt{FeaturePipeline} fitting inside cross-validation folds and fixed random seed 42.
    \item \textbf{External Validity}: Telemetry characteristics reflect commercial heavy-duty trucks; deployment to light electric vehicles requires domain recalibration.
\end{itemize}
...
\bibitem{roslan2024} M. A. Roslan \textit{et al.}, ``A bibliometric analysis of predictive maintenance in smart manufacturing,'' \textit{Journal of Manufacturing Systems}, vol. 72, pp. 112--128, 2024.
\bibitem{akarte2018} R. Akarte and N. Hemachandra, ``Cost-sensitive learning for imbalanced data in predictive maintenance,'' \textit{IEEE Transactions on Industrial Informatics}, vol. 14, no. 10, pp. 4520--4529, 2018.
\bibitem{bifet2007} A. Bifet and R. Gavalda, ``Learning from time-changing data with adaptive windowing,'' in \textit{Proc. SIAM Int. Conf. Data Mining (SDM)}, 2007, pp. 443--448.
\bibitem{chen2016} T. Chen and C. Guestrin, ``XGBoost: A scalable tree boosting system,'' in \textit{Proc. 22nd ACM SIGKDD Int. Conf. Knowledge Discovery Data Mining}, 2016, pp. 785--794.
\bibitem{lu2019} J. Lu, A. Liu, F. Dong, F. Gu, J. Gama, and G. Zhang, ``Learning under concept drift: A review,'' \textit{IEEE Trans. Knowledge Data Eng.}, vol. 31, no. 12, pp. 2346--2363, 2019.
\bibitem{lundberg2017} S. M. Lundberg and S.-I. Lee, ``A unified approach to interpreting model predictions,'' in \textit{Adv. Neural Inf. Process. Syst. (NeurIPS)}, vol. 30, 2017, pp. 4765--4774.
\bibitem{mothilal2020} R. K. Mothilal, A. Sharma, and C. Tan, ``Explaining machine learning classifiers through diverse counterfactual explanations,'' in \textit{Proc. ACM Conf. Fairness, Accountability, Transparency (FAT*)}, 2020, pp. 607--617.
\bibitem{tzelepis2025} G. Tzelepis, ``Multi-detector consensus ensembles for online concept drift detection,'' \textit{IEEE Trans. Neural Netw. Learn. Syst.}, vol. 36, no. 4, pp. 1890--1902, 2025.
\bibitem{zemmouchi2026} L. Zemmouchi-Ghomari, ``Explainable artificial intelligence in predictive maintenance: A systematic review,'' \textit{Computers in Industry}, vol. 154, p. 104080, 2026.
\end{thebibliography}
